	...
	export default {
		data () {
			return {
				...
				items : [],
				fields : [
					{fieldname : 'sku', caption : 'SKU', width: 150},
					{fieldname : 'name', caption : 'Product Name', width: null},
					{fieldname : 'category', caption : 'Category', width: 150},
					{fieldname : 'uom', caption : 'UOM', width: 100}, //hiddenfilter : true},
					{fieldname : 'price', caption : 'Price', width: 120},
					{fieldname : 'tax', caption : 'Tax %', width: 100},
				],
				error : {
					status : false,
					title : 'Error Occured',
		...
		methods:{
			...
			refreshData(reset){
				...
				var url = CONFIG.rest_url + '/productof/' + this.selectedCompany.id
			...
			deleteData(item){
				var id = item.id;
				var vm = this;
				axios.delete(CONFIG.rest_url + '/product/' + id)
				.then(function(response) {
